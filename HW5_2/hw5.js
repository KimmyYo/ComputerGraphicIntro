...
function groundTri(a, b, c, check){
    var texIndice = [];
    if(check == 0) texIndice = [0, 1, 2];
    else texIndice = [0, 2, 3];
    groundPointsArray.push(a);
    groundPointsArray.push(b);
    groundPointsArray.push(c);

     // normals are vectors

    groundNormalsArray.push(a[0],a[1], a[2]);
    groundNormalsArray.push(b[0],b[1], b[2]);
    groundNormalsArray.push(c[0],c[1], c[2]);

    groundTexCoordsArray.push(texCoord[texIndice[0]], texCoord[texIndice[1]], texCoord[texIndice[2]])
...
function squareGround(){
    ...
    groundTri(vertices[0], vertices[1], vertices[2], 0);
    groundTri(vertices[0], vertices[2], vertices[3], 1);
...
window.onload = function init() {
    ...
    projectionMatrix = perspective(90.0, 1.0, 0.2, 2000);
